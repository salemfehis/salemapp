...
# =========================================================
    ...
    with c2:
        nb_dp = st.number_input("Nombre de décanteurs", 1, 4, 2, key="nb_dp")

    if horizon_dp == "2036 (+10 ans)":
        Kpts_dp, Qpts_s_dp, _ = qp(T10[2])
        Qpts_dp   = Qpts_s_dp * 3600          # m³/h — temps sec
        Qptp_dp   = Qptp10[1]                 # m³/h — temps de pluie
        Qmoy_h_dp = T10[0] / 24               # m³/h — débit moyen horaire
    else:
    ...
    # ── Vitesse limite selon K = Qpts / Qmoy,h ──────────────
    K_dp = Qpts_dp / Qmoy_h_dp
    if   K_dp <= 2.5: Vlim_dp = 2.0
    elif K_dp <= 3.0: Vlim_dp = 2.5
    elif K_dp <= 5.0: Vlim_dp = 3.75
    elif K_dp <= 8.0: Vlim_dp = 5.0
    else:             Vlim_dp = 6.0

    st.markdown("#### 1 · Débits de référence")
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Qmoy,h (m³/h)",  f"{Qmoy_h_dp:.2f}")
    c2.metric("Qpts ts (m³/h)", f"{Qpts_dp:.2f}")
    c3.metric("Qptp (m³/h)",    f"{Qptp_dp:.2f}")
    c4.metric("K = Qpts/Qmoy",  f"{K_dp:.2f}")

    st.info(f"Vlim retenue = **{Vlim_dp} m/h**  (K = {K_dp:.2f})")

    # ── Surface horizontale ── dimensionnée sur Qptp ─────────
    st.markdown("#### 2 · Surface horizontale  —  basée sur **Qptp**")
    Sh_dp_total = Qptp_dp / Vlim_dp
    Sh_dp_unit  = Sh_dp_total / nb_dp

    c1, c2 = st.columns(2)
    c1.metric("Sh totale (m²)",  f"{Sh_dp_total:.2f}")
    c2.metric("Sh unitaire (m²)", f"{Sh_dp_unit:.2f}")

    # ── Volume et géométrie ── dimensionnés sur Qptp ─────────
    st.markdown("#### 3 · Volume & Géométrie  —  basés sur **Qptp**")
    ts_dp   = st.slider("Temps de séjour Ts (h)", 1.0, 2.0, 1.5, 0.1, key="ts_dp_s")
    V_dp_total = Qptp_dp * ts_dp          # m³
    V_dp_unit  = V_dp_total / nb_dp
    H_dp       = V_dp_unit / Sh_dp_unit
    Ht_dp      = H_dp + 0.75              # revanche 0.75 m
    D_dp       = math.sqrt((4 * Sh_dp_unit) / math.pi)

    c1, c2, c3 = st.columns(3)
    c1.metric("Volume total (m³)",    f"{V_dp_total:.2f}")
    c2.metric("Volume unitaire (m³)", f"{V_dp_unit:.2f}")
    c3.metric("Diamètre D (m)",       f"{D_dp:.2f}")

    df_dp = pd.DataFrame({
        "Paramètre": [
            "Qmoy,h (m³/h)", "Qpts temps sec (m³/h)", "Qptp temps pluie (m³/h)",
            "K = Qpts/Qmoy", "Vlim (m/h)",
            "Surface totale (m²)", "Surface unitaire (m²)",
            "Ts (h)", "Volume total (m³)", "Volume unitaire (m³)",
            "Hauteur eau H (m)", "Revanche (m)", "Hauteur totale Ht (m)",
            "Diamètre D (m)"
        ],
        "Valeur": [
            Qmoy_h_dp, Qpts_dp, Qptp_dp,
            K_dp, Vlim_dp,
            Sh_dp_total, Sh_dp_unit,
            ts_dp, V_dp_total, V_dp_unit,
            H_dp, 0.75, Ht_dp,
            D_dp
        ],
        "Débit utilisé": [
    ...
    st.success(f"✔ Décanteur Ø {D_dp:.2f} m  ×  Ht {Ht_dp:.2f} m  ×  {nb_dp} unité(s)")
